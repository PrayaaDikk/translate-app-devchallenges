import Radio from "@/components/ui/Radio";
import { Language } from "@/types/index.t";
import { useTranslate } from "@/hooks/useTranslate";

type FormHeaderProps = {
    languageList: Language[];
    isTranslatingForm?: boolean;
};

const FormHeader = ({ languageList, isTranslatingForm }: FormHeaderProps) => {
    const {
        sourceLang,
        targetLang,
        setSourceLang,
        setTargetLang,
        swapLanguages,
    } = useTranslate();

    const renderRadioGroup = ({
        groupName,
        selectedCode,
        onChange,
        includeDetect = false,
    }: {
        groupName: string;
        selectedCode: string;
        onChange: (code: string) => void;
        includeDetect?: boolean;
    }) => (
        <div className="flex items-center gap-6">
            {includeDetect && (
                <Radio
                    label="Detect Language"
                    code=""
                    disabled
                    checked={selectedCode === ""}
                    onChange={() => {}}
                    id={`${groupName}-detect`}
                    nameType={groupName}
                />
            )}
            {languageList.map(({ code, label, dropdown }) => (
                <Radio
                    key={code}
                    code={code}
                    label={label}
                    checked={code === selectedCode}
                    onChange={onChange}
                    dropdown={dropdown}
                    id={`${groupName}-${code}`}
                    nameType={groupName}
                />
            ))}
        </div>
    );

    return isTranslatingForm ? (
        <header>
            {renderRadioGroup({
                groupName: "translating",
                selectedCode: sourceLang,
                onChange: setSourceLang,
                includeDetect: true,
            })}
        </header>
    ) : (
        <header className="flex items-center justify-between">
            {renderRadioGroup({
                groupName: "translated",
                selectedCode: targetLang,
                onChange: setTargetLang,
            })}
            <div
                className="border-darkGray1 cursor-pointer rounded-2xl border-2 p-2"
                onClick={swapLanguages}
            >
                {/* icon swap */}
                <svg
                    className="size-6"
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M1.6665 6.66671L0.959397 5.9596L0.25229 6.66671L0.959397 7.37381L1.6665 6.66671ZM17.4998 7.66671C18.0521 7.66671 18.4998 7.21899 18.4998 6.66671C18.4998 6.11442 18.0521 5.66671 17.4998 5.66671V7.66671ZM4.29273 2.62627L0.959397 5.9596L2.37361 7.37381L5.70694 4.04048L4.29273 2.62627ZM0.959397 7.37381L4.29273 10.7071L5.70694 9.29293L2.37361 5.9596L0.959397 7.37381ZM1.6665 7.66671H17.4998V5.66671H1.6665V7.66671Z"
                        fill="#4D5562"
                    />
                    <path
                        d="M18.3335 13.3333L19.0406 12.6262L19.7477 13.3333L19.0406 14.0404L18.3335 13.3333ZM10.8335 14.3333C10.2812 14.3333 9.8335 13.8856 9.8335 13.3333C9.8335 12.781 10.2812 12.3333 10.8335 12.3333L10.8335 14.3333ZM15.7073 9.29289L19.0406 12.6262L17.6264 14.0404L14.2931 10.7071L15.7073 9.29289ZM19.0406 14.0404L15.7073 17.3738L14.2931 15.9596L17.6264 12.6262L19.0406 14.0404ZM18.3335 14.3333L10.8335 14.3333L10.8335 12.3333L18.3335 12.3333L18.3335 14.3333Z"
                        fill="#4D5562"
                    />
                </svg>
            </div>
        </header>
    );
};

export default FormHeader;
